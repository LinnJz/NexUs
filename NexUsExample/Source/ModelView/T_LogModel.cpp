...
int T_LogModel::rowCount(const QModelIndex& parent) const { return this->_logList.count(); }

QVariant T_LogModel::data(const QModelIndex& index, int role) const
{
  if (role == Qt::DisplayRole) { return _logList[index.row()]; }
  return QVariant();
}
...
QStringList T_LogModel::getLogList() const { return this->_logList; }
