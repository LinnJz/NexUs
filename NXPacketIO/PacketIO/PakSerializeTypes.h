...
template <>
inline void NX_PACKETIO_EXPORT Load(PakI& aAr, bool& aValue)
{
    char val(0);
    aAr.Serialize(val);
    aValue = (val != 0);
...
template <>
inline void NX_PACKETIO_EXPORT Save(PakO& aAr, bool& aValue)
{
    char val = aValue ? 1 : 0;
    aAr.Serialize(val);
}
...
// std::string
void NX_PACKETIO_EXPORT Save(PakO& aAr, std::string& aValue);
void NX_PACKETIO_EXPORT Load(PakI& aAr, std::string& aValue);
