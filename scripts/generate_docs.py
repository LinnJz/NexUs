...
HEADER_DIR = Path(__file__).resolve().parent.parent / "NexUs" / "Source" / "include"
OUTPUT_FILE = Path(__file__).resolve().parent.parent / "skills" / "references" / "API.md"

# 跳过的内部文件
SKIP_FILES = {
    "NXProperty.h", "NXSingleton.h", "NXDef.h",
    "NXExponentialBlur.h",
}
...
# class NX_EXPORT ClassName : public BaseClass
RE_CLASS = re.compile(
    r"class\s+NX_EXPORT\s+(\w+)\s*:\s*public\s+(\w+)"
)
...
def main():
    ...
    for h in headers:
        if h.name in SKIP_FILES:
            continue
        if h.name.startswith("NX") and h.suffix == ".h":
            info = parse_header(h)
            if info:
                classes.append(info)
    ...
    priority = [
        "NXApplication", "NXTheme", "NXWindow", "NXWidget",
        "NXAppBar", "NXNavigationBar", "NXNavigationRouter",
    ]
    def sort_key(c):
