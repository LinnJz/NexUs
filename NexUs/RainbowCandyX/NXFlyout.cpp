...
#include "NXFlyoutPrivate.h"
#include "NXTheme.h"
Q_PROPERTY_CREATE_CPP(NXFlyout, int, BorderRadius)
Q_PROPERTY_CREATE_CPP(NXFlyout, QString, Title)
Q_PROPERTY_CREATE_CPP(NXFlyout, QString, Content)
Q_PROPERTY_CREATE_CPP(NXFlyout, bool, IsLightDismiss)

NXFlyout::NXFlyout(QWidget *parent)
	: QWidget{nullptr}, d_ptr(new NXFlyoutPrivate())
{
	Q_D(NXFlyout);
	d->q_ptr = this;
	d->_pBorderRadius = 8;
	d->_pIsLightDismiss = true;
	setObjectName("NXFlyout");
...
void NXFlyout::setContentWidget(QWidget *widget)
{
	Q_D(NXFlyout);
...
void NXFlyout::showFlyout(QWidget *target)
{
	Q_D(NXFlyout);
	if (!target)
	{
...
void NXFlyout::paintEvent(QPaintEvent *event)
{
	Q_D(NXFlyout);
...
NXFlyoutPrivate::NXFlyoutPrivate(QObject *parent)
	: QObject(parent)
{
}

NXFlyoutPrivate::~NXFlyoutPrivate()
{
}

bool NXFlyoutPrivate::eventFilter(QObject *watched, QEvent *event)
{
	Q_Q(NXFlyout);
...
void NXFlyoutPrivate::_doShowAnimation()
{
