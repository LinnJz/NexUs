...
void
NXRatingControl::setRating(int Rating)
{
  Q_D(NXRatingControl);
...
int
NXRatingControl::getRating() const
{
...
NXRatingControl::NXRatingControl(QWidget *parent)
    : QWidget { parent }
    , d_ptr(new NXRatingControlPrivate())
{
  Q_D(NXRatingControl);
  d->q_ptr        = this;
  d->_pMaxRating  = 5;
...
void
NXRatingControl::paintEvent(QPaintEvent *event)
{
  Q_UNUSED(event);
  Q_D(NXRatingControl);
...
void
NXRatingControl::mouseMoveEvent(QMouseEvent *event)
{
  Q_D(NXRatingControl);
  if (d->_pIsReadOnly)
  {
...
void
NXRatingControl::mousePressEvent(QMouseEvent *event)
{
  Q_D(NXRatingControl);
  if (!d->_pIsReadOnly && d->_hoveredStar >= 0)
  {
...
void
NXRatingControl::leaveEvent(QEvent *event)
{
  Q_D(NXRatingControl);
  d->_hoveredStar = -1;
  update();
...
QSize
NXRatingControl::sizeHint() const
{
