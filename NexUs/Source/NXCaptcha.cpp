...
int
NXCaptcha::getCodeLength() const
{
  return d_ptr->_pCodeLength;
}

void
NXCaptcha::setBoxSize(int BoxSize)
{
  Q_D(NXCaptcha);
...
int
NXCaptcha::getBoxSize() const
{
  return d_ptr->_pBoxSize;
}

void
NXCaptcha::setInputMode(InputMode mode)
{
  Q_D(NXCaptcha);
...
bool
NXCaptcha::eventFilter(QObject *watched, QEvent *event)
{
  Q_D(NXCaptcha);
  ...
  if (event->type() == QEvent::KeyPress)
  {
    ...
    if (keyEvent->matches(QKeySequence::Paste))
    {
      ...
      {
        clipText.remove(QRegularExpression(QStringLiteral("[^0-9]")));
      }
      else
      {
        clipText.remove(QRegularExpression(QStringLiteral("[^A-Z0-9]")));
      }
      if (!clipText.isEmpty())
      {
...
void
NXCaptchaPrivate::onThemeChanged(NXThemeType::ThemeMode themeMode)
{
...
void
NXCaptchaPrivate::_updateValidators()
{
  Q_Q(NXCaptcha);
  QString pattern = (_inputMode == NXCaptcha::DigitOnly) ? QStringLiteral("[0-9]") : QStringLiteral("[A-Za-z0-9]");
...
void
NXCaptchaPrivate::_applyThemeStyle()
{
...
void
NXCaptchaPrivate::_applyBoxStyle(QLineEdit *box, bool focused)
{
  ...
  box->setStyleSheet(QString(QStringLiteral("QLineEdit { background: %1; border: 1px solid %2; border-radius: %3px; "
