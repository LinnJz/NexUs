...
void
NXCommandBar::addItem(const CommandItem &item)
{
  Q_D(NXCommandBar);
...
void
NXCommandBar::paintEvent(QPaintEvent *event)
{
  Q_UNUSED(event);
  Q_D(NXCommandBar);
...
void
NXCommandBar::mouseMoveEvent(QMouseEvent *event)
{
  Q_D(NXCommandBar);
  QList<QRect> rects = d->_getVisibleItemRects();
  bool hasOverflow   = (d->_visibleCount < d->_items.count());
...
void
NXCommandBar::mousePressEvent(QMouseEvent *event)
{
  Q_D(NXCommandBar);
...
void
NXCommandBar::mouseReleaseEvent(QMouseEvent *event)
{
  Q_D(NXCommandBar);
  if (event->button() == Qt::LeftButton)
  {
...
void
NXCommandBar::leaveEvent(QEvent *event)
{
  Q_D(NXCommandBar);
  d->_hoveredIndex = -1;
  update();
...
int
NXCommandBarPrivate::_getItemWidth(const NXCommandBar::CommandItem &item) const
{
  if (item.isSeparator)
  {
    return 8;
...
QList<QRect>
NXCommandBarPrivate::_getVisibleItemRects() const
{
  QList<QRect> rects;
