...
Q_PROPERTY_CREATE_CPP(NXTransfer, QS_SET_CREF(QString), SourceTitle)
Q_PROPERTY_CREATE_CPP(NXTransfer, QS_SET_CREF(QString), TargetTitle)
...
NXTransfer::NXTransfer(QWidget *parent)
    : QWidget(parent)
    , d_ptr(new NXTransferPrivate())
{
  Q_D(NXTransfer);
  d->q_ptr             = this;
  d->_pBorderRadius    = 6;
...
void
NXTransfer::setSourceItems(const QStringList &items)
{
  Q_D(NXTransfer);
...
void
NXTransfer::addSourceItem(const QString &text)
{
  Q_D(NXTransfer);
  QStandardItem *item = new QStandardItem(text);
...
void
NXTransfer::addSourceItems(const QStringList &items)
{
  Q_D(NXTransfer);
  QColor textColor = NXThemeColor(d->_themeMode, BasicText);
  for (const QString &text : items)
...
QStringList
NXTransfer::getSourceItems() const
{
  QStringList items;
...
QStringList
NXTransfer::getTargetItems() const
{
  QStringList items;
...
void
NXTransfer::paintEvent(QPaintEvent *event)
{
  Q_D(NXTransfer);
