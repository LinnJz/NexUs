...
void NXReminderCard::paintEvent(QPaintEvent *event)
{
  ...
  if (!d->_pCardPixmap.isNull())
  {
    ...
    if (d->_pCardPixMode == NXCardPixType::PixMode::Ellipse)
    {
      ...
      painter.drawPixmap(QRect(pixLeftMargin + d->_shadowBorderWidth,
    ...
    {
      painter.drawPixmap(pixLeftMargin + d->_shadowBorderWidth,
                         (height() - d->_pCardPixmapSize.height()) / 2,
                         d->_pCardPixmapSize.width(),
                         d->_pCardPixmapSize.height(),
                         d->_pCardPixmap);
    ...
    {
      path.addRoundedRect(QRectF(pixLeftMargin,
                                 (height() - d->_pCardPixmapSize.height()) / 2,
                                 d->_pCardPixmapSize.width(),
                                 d->_pCardPixmapSize.height()),
  ...
  painter.drawText(QRect(textStartX, foregroundRect.y(), textWidth, this->height() / 2 - d->_pTitleSpacing),
                   Qt::AlignLeft | Qt::AlignBottom | Qt::TextWordWrap | Qt::TextWrapAnywhere,
                   d->_pTitle);
  // 副标题
