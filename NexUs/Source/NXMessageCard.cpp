...
NXMessageCard::NXMessageCard(QWidget *parent)
    : QWidget(parent)
    , d_ptr(new NXMessageCardPrivate())
{
  Q_D(NXMessageCard);
  d->q_ptr = this;
...
void NXMessageCard::paintEvent(QPaintEvent *event)
{
  ...
  bubblePath.addRoundedRect(
      QRectF(bubbleLeft, bubbleTop, bubbleWidth, bubbleHeight), d->_pBorderRadius, d->_pBorderRadius);
  ...
  painter.drawText(
      QRect(bubbleLeft + 40, bubbleTop + 8, bubbleWidth - 50, 22), Qt::AlignLeft | Qt::AlignVCenter, d->_pTitle);
  ...
  painter.setPen(NXThemeColor(d->_themeMode, BasicText));
  painter.drawText(QRect(bubbleLeft + 40, bubbleTop + 32, bubbleWidth - 50, bubbleHeight - 40),
