  ...
  case QStyle::CC_ToolButton :
  {
    ...
    if (const QStyleOptionToolButton *bopt = qstyleoption_cast<const QStyleOptionToolButton *>(option))
    {
      ...
      if (bopt->state.testFlag(QStyle::State_Enabled))
      {
        ...
        {
          painter->setBrush(_pIsTransparent ? NXThemeColor(_themeMode, BasicPressAlpha)
                                            : NXThemeColor(_themeMode, BasicPress));
        ...
        else
        {
          if (_pIsSelected)
          {
            painter->setBrush(_pIsTransparent ? NXThemeColor(_themeMode, BasicSelectedAlpha)
          ...
          else
          {
            ...
            {
              painter->setBrush(_pIsTransparent ? NXThemeColor(_themeMode, BasicHoverAlpha)
                                                : NXThemeColor(_themeMode, BasicHover));
              painter->drawRoundedRect(toolButtonRect, _pBorderRadius, _pBorderRadius);
            }
            else
            {
              if (!_pIsTransparent)
              {
                ...
                painter->drawLine(toolButtonRect.x() + _pBorderRadius,
                                          ...
                                          const QWidget *widget) const
{
  if (type == QStyle::CT_ToolButton)
  {
    if (const QStyleOptionToolButton *bopt = qstyleoption_cast<const QStyleOptionToolButton *>(option))
    {
      ...
      {
        toolButtonSize.setWidth(toolButtonSize.width() + _contentMargin +
                                       ...
                                       const QWidget *widget) const
{
  ...
  else if (bopt->features.testFlag(QStyleOptionToolButton::HasMenu))
  {
    ...
    QRect expandIconRect(toolButtonRect.right() - _contentMargin - indicatorWidth,
                         toolButtonRect.y() + 1,
                         indicatorWidth,
    ...
    if (widget->property("NXIconType").toString().isEmpty())
    {
      ...
      if (!icon.isNull())
      {
        QPixmap iconPix = icon.pixmap(iconSize,
                                      (bopt->state & State_Enabled) ? QIcon::Normal : QIcon::Disabled,
        ...
        {
          painter->drawPixmap(
              QRect(QPoint(iconRect.x() + _contentMargin, iconRect.center().y() - iconSize.height() / 2), iconSize),
              iconPix);
          break;
        }
        case Qt::ToolButtonTextUnderIcon :
        {
          ...
          // 图标区域
          QRectF adjustIconRect(buttonRect.center().x() - iconSize.width() / 2,              // 水平居中
    ...
    else
    {
      ...
      case Qt::ToolButtonTextUnderIcon :
      {
        ...
        if (bopt->features.testFlag(QStyleOptionToolButton::HasMenu) &&
            !bopt->features.testFlag(QStyleOptionToolButton::MenuButtonPopup))
        {
          adjustIconRect.setLeft(
              buttonRect.left() + _contentMargin + textWidth / 2 /*ratio*/ -
              painter->fontMetrics().height() /
                  2); /*setRight(adjustIconRect.right()  +10 - _calculateExpandIndicatorWidth(bopt, painter));*/
...
void NXToolButtonStyle::_drawText(QPainter *painter, QRect contentRect, const QStyleOptionToolButton *bopt) const
{
  if (!bopt->text.isEmpty())
  {
    ...
    case Qt::ToolButtonTextBesideIcon :
    {
      painter->drawText(QRect(contentRect.x() + _contentMargin * 2 + bopt->iconSize.width(),
                              contentRect.y(),
                              contentRect.width() - bopt->iconSize.width(),
                              contentRect.height()),
    ...
    case Qt::ToolButtonTextUnderIcon :
    {
      ...
      QRectF textRect(bopt->rect.left(), bopt->rect.top() + bopt->rect.height() * 0.65, bopt->rect.width(), textHeight);
      if (bopt->features.testFlag(QStyleOptionToolButton::HasMenu) &&
          !bopt->features.testFlag(QStyleOptionToolButton::MenuButtonPopup))
      {
        textRect.setLeft(textRect.left() + _contentMargin);
        painter->drawText(textRect, Qt::AlignLeft, bopt->text);
        // painter->drawText(QPointF(contentRect.left() + 1, contentRect.bottom() - metrics.height() * 0.3),
        // bopt->text);
      }
      else
      {
        painter->drawText(textRect, Qt::AlignHCenter, bopt->text);
        // painter->drawText(QPointF(contentRect.left() + 1 + (contentRect.width() - textWidth) / 2,
        // contentRect.bottom() - metrics.height() * 0.3 - bopt->rect.height() * 0.1), bopt->text);
      }
      break;
    }
    case Qt::ToolButtonFollowStyle :
