        ...
        else
        {
          ...
          if (isHover)
          {
            ...
            painter->drawRoundedRect(
                QRectF(hoverRect.x() + 3, hoverRect.y() + heightOffset, 3, hoverRect.height() - 2 * heightOffset),
                3,
                3);
          }
          // Icon绘制
          ...
          else
          {
            ...
            {
              painter->save();
              isHover ? painter->setPen(!mopt->state.testFlag(QStyle::State_Enabled)
                                            ? Qt::gray
                                            : NXThemeColor(_themeMode, PrimaryPress))
                      : painter->setPen(!mopt->state.testFlag(QStyle::State_Enabled) ? Qt::gray
                                        : _themeMode == NXThemeType::Light           ? Qt::black
                                                                                     : Qt::white);
            ...
            else
            {
              if (!menuIcon.isNull())
              {
                painter->drawPixmap(
                    QRect(
                        menuRect.x() + contentPadding, menuRect.center().y() - _iconWidth / 2, _iconWidth, _iconWidth),
