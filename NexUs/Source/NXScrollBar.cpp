...
void NXScrollBar::contextMenuEvent(QContextMenuEvent *event)
{
  ...
  menu->addSeparator();
  // Left edge Top
  QAction *actScrollTop = menu->addNXIconAction(horiz ? NXIconType::ArrowLeftToLine : NXIconType::ArrowUpToLine,
                                                horiz ? tr("左边缘") : tr("顶端"));
  // Right edge Bottom
  QAction *actScrollBottom = menu->addNXIconAction(horiz ? NXIconType::ArrowRightToLine : NXIconType::ArrowDownToLine,
                                                   horiz ? tr("右边缘") : tr("底部"));
  menu->addSeparator();
  // Page left Page up
