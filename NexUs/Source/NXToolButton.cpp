...
{
  if (event->button() == Qt::RightButton) { Q_EMIT rightReleased(); }
  else if (event->button() == Qt::MiddleButton) { Q_EMIT middleReleased(); }
  QToolButton::mouseReleaseEvent(event);
}

void NXAdvancedToolButton::mousePressEvent(QMouseEvent *event)
{
  if (event->button() == Qt::RightButton) { Q_EMIT rightPressed(); }
  else if (event->button() == Qt::MiddleButton) { Q_EMIT middlePressed(); }
  QToolButton::mousePressEvent(event);
}

NXToolButton::NXToolButton(QWidget *parent)
    : NXAdvancedToolButton(parent)
    , d_ptr(new NXToolButtonPrivate())
{
  Q_D(NXToolButton);
  d->q_ptr = this;
