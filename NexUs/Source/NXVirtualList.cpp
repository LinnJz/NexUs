...
#include "DeveloperComponents/NXVirtualListStyle.h"
...
NXVirtualList::NXVirtualList(QWidget *parent)
    : QListView(parent)
    , d_ptr(new NXVirtualListPrivate())
{
  Q_D(NXVirtualList);
  d->q_ptr                    = this;
  d->_pItemHeight             = 40;
...
void
NXVirtualList::setItemCount(int count)
{
  Q_D(NXVirtualList);
  d->_itemCount = count;
}
...
int
NXVirtualList::getItemCount() const
{
  return d_ptr->_itemCount;
}

void
NXVirtualList::paintEvent(QPaintEvent *event)
{
  Q_D(NXVirtualList);
