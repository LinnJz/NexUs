...
NXSelectorBar::NXSelectorBar(QWidget *parent)
    : QWidget { parent }
    , d_ptr(new NXSelectorBarPrivate())
{
  Q_D(NXSelectorBar);
  d->q_ptr          = this;
  d->_pCurrentIndex = 0;
...
void
NXSelectorBar::addItem(const QString &text)
{
  addItem(NXIconType::None, text);
}

void
NXSelectorBar::addItem(NXIconType::IconName icon, const QString &text)
{
  Q_D(NXSelectorBar);
...
int
NXSelectorBar::getItemCount() const
{
  return d_ptr->_items.count();
}

QSize
NXSelectorBar::sizeHint() const
{
...
void
NXSelectorBar::paintEvent(QPaintEvent *event)
{
  Q_UNUSED(event)
  Q_D(NXSelectorBar);
  ...
  for (int i = 0; i < itemCount; ++i)
  {
    ...
    if (item.icon != NXIconType::None)
    {
      ...
      painter.setFont(textFont);
      painter.drawText(QPointF(startX + iconWidth + 4, itemRect.center().y() + textFm.ascent() / 2.0 - 1), item.text);
    }
    else
    {
...
void
NXSelectorBar::resizeEvent(QResizeEvent *event)
{
  Q_D(NXSelectorBar);
  if (!d->_items.isEmpty())
  {
...
void
NXSelectorBar::mousePressEvent(QMouseEvent *event)
{
  Q_D(NXSelectorBar);
...
void
NXSelectorBar::mouseMoveEvent(QMouseEvent *event)
{
  Q_D(NXSelectorBar);
  int index = d->_itemIndexAtPos(event->pos());
...
void
NXSelectorBar::leaveEvent(QEvent *event)
{
  Q_D(NXSelectorBar);
  d->_hoveredIndex = -1;
  update();
...
void
NXSelectorBarPrivate::_startIndicatorAnimation(qreal targetX, qreal targetWidth)
{
  Q_Q(NXSelectorBar);
...
int
NXSelectorBarPrivate::_itemIndexAtPos(QPoint pos) const
{
