...
void
NXLineEditPrivate::resetTextRoute() noexcept
{
...
void
NXLineEditPrivate::pushTextRoute() noexcept
{
  _overallRedoRouteIndex = -1;
  ...
  if (_overallRedoRouteIndex >= 0)
  {
    _overallRedoRouteIndex = std::max(-1, _overallRedoRouteIndex - removeCount);
  }
}

bool
NXLineEditPrivate::canTextRouteBack() const noexcept
{
  return _textRouteCurrentIndex > 0;
}

bool
NXLineEditPrivate::canTextRouteForward() const noexcept
{
  return _textRouteCurrentIndex + 1 < _textRouteList.size();
}

void
NXLineEditPrivate::textRouteBack() noexcept
{
  if (!canTextRouteBack())
  {
    return;
...
void
NXLineEditPrivate::textRouteForward() noexcept
{
  if (!canTextRouteForward())
  {
    return;
  }
  _textRouteCurrentIndex += 1;
...
bool
NXLineEditPrivate::canOverallUndo() const noexcept
{
...
bool
NXLineEditPrivate::canOverallRedo() const noexcept
{
  if (_textRouteList.isEmpty())
  {
    return false;
...
void
NXLineEditPrivate::overallUndo() noexcept
{
  if (!canOverallUndo())
  {
    return;
...
void
NXLineEditPrivate::overallRedo() noexcept
{
  if (!canOverallRedo())
  {
    return;
...
void
NXLineEditPrivate::onWMWindowClickedEvent(const QVariantMap &data)
{
  Q_Q(NXLineEdit);
...
void
NXLineEditPrivate::onThemeChanged(NXThemeType::ThemeMode themeMode) noexcept
{
  Q_Q(NXLineEdit);
...
NXLineEditPrivate::TextRouteState
NXLineEditPrivate::_currentTextRouteState() const noexcept
{
  Q_Q(const NXLineEdit);
  ...
  {
    state.selectionLength = q->selectedText().size();
  }
  return state;
}

void
NXLineEditPrivate::_applyTextRouteState(const TextRouteState &state) noexcept
{
  Q_Q(NXLineEdit);
