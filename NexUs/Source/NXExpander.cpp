...
Q_PROPERTY_CREATE_CPP(NXExpander, QS_SET_CREF(QString), Title)
Q_PROPERTY_CREATE_CPP(NXExpander, QS_SET_CREF(QString), SubTitle)
...
void
NXExpanderPrivate::_toggleExpand()
{
...
void
NXExpanderPrivate::_startExpandAnimation(bool expand)
{
  Q_Q(NXExpander);
  if (!_contentWidget)
  {
  ...
  _contentHeight = _contentWidget->sizeHint().height() + _contentLayout->contentsMargins().top() +
                   _contentLayout->contentsMargins().bottom();
...
void
NXExpander::setExpandDirection(ExpandDirection direction)
{
  Q_D(NXExpander);
...
NXExpander::ExpandDirection
NXExpander::getExpandDirection() const
{
  return d_ptr->_expandDirection;
}

void
NXExpander::setContentWidget(QWidget *widget)
{
  Q_D(NXExpander);
  if (d->_contentWidget)
  {
...
QWidget *
NXExpander::getContentWidget() const
{
  return d_ptr->_contentWidget;
}

void
NXExpander::setHeaderWidget(QWidget *widget)
{
  Q_D(NXExpander);
...
void
NXExpander::setIsExpanded(bool expanded)
{
  Q_D(NXExpander);
...
bool
NXExpander::getIsExpanded() const
{
  return d_ptr->_isExpanded;
}

void
NXExpander::paintEvent(QPaintEvent *event)
{
  Q_UNUSED(event);
  Q_D(NXExpander);
  ...
  QFont arrowFont = QFont(QStringLiteral("NXAwesome"));
...
void
NXExpander::mousePressEvent(QMouseEvent *event)
{
  Q_D(NXExpander);
  int headerY = (d->_expandDirection == Down) ? 0 : height() - d->_headerHeight;
...
void
NXExpander::mouseReleaseEvent(QMouseEvent *event)
{
  Q_D(NXExpander);
...
bool
NXExpander::event(QEvent *event)
{
  switch (event->type())
  {
  case QEvent::Enter :
  case QEvent::Leave : update(); break;
  default            : break;
  }
