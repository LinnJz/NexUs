...
NXIconButton::NXIconButton(
    NXIconType::IconName awesome, int pixelSize, int fixedWidth, int fixedHeight, QWidget *parent)
    : QPushButton(parent)
    , d_ptr(new NXIconButtonPrivate())
{
  Q_D(NXIconButton);
...
NXIconType::IconName NXIconButton::getAwesome() const { return this->d_ptr->_pAwesome; }
...
void NXIconButton::paintEvent(QPaintEvent *event)
{
  ...
  if (d->_isAlphaAnimationFinished || d->_pIsSelected)
  {
    painter.setBrush(d->_pIsSelected ? d->_themeMode == NXThemeType::Light ? d->_pLightHoverColor : d->_pDarkHoverColor
                     : isEnabled()   ? underMouse() ? d->_themeMode == NXThemeType::Light ? d->_pLightHoverColor
                                                                                          : d->_pDarkHoverColor
                                                    : Qt::transparent
                                     : Qt::transparent);
  }
