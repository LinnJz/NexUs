...
Q_PROPERTY_CREATE_CPP(NXSpotlight, QS_SET_CREF(QString), Title)
Q_PROPERTY_CREATE_CPP(NXSpotlight, QS_SET_CREF(QString), Content)
...
NXSpotlight::NXSpotlight(QWidget *parent)
    : QWidget(parent)
    , d_ptr(new NXSpotlightPrivate())
{
  Q_D(NXSpotlight);
  d->q_ptr           = this;
  d->_pBorderRadius  = 8;
  ...
  d->_prevButton = new NXPushButton(QStringLiteral("上一步"), d->_tipWidget);
  ...
  d->_nextButton = new NXPushButton(QStringLiteral("下一步"), d->_tipWidget);
...
void
NXSpotlight::showSpotlight(QWidget *target, const QString &buttonText)
{
  Q_D(NXSpotlight);
  SpotlightStep step;
  step.target   = target;
...
void
NXSpotlight::setSteps(const QList<SpotlightStep> &steps)
{
  Q_D(NXSpotlight);
  d->_steps = steps;
}
...
NXSpotlight::start()
{
  ...
  connect(fadeIn, &QPropertyAnimation::valueChanged, this, [=]()
  {
    update();
  });
...
int
NXSpotlight::currentStep() const
{
  return d_ptr->_currentStep;
}

int
NXSpotlight::stepCount() const
{
  return d_ptr->_steps.size();
}

void
NXSpotlight::paintEvent(QPaintEvent *event)
{
  Q_D(NXSpotlight);
...
void
NXSpotlight::mousePressEvent(QMouseEvent *event)
{
  Q_D(NXSpotlight);
  QRectF spotRect = d->_pSpotlightRect;
...
bool
NXSpotlight::eventFilter(QObject *watched, QEvent *event)
{
  Q_D(NXSpotlight);
