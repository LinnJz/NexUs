...
bool ElaThemePrivate::eventFilter(QObject *watched, QEvent *event)
{
	if (_isFollowSystemTheme && event->type() == QEvent::ApplicationPaletteChange)
	{
		_applySystemTheme();
	}
...
ElaThemeType::ThemeMode ElaThemePrivate::_detectSystemThemeMode() const
{
	...
	return windowColor.lightness() < 128 ? ElaThemeType::Dark : ElaThemeType::Light;
}

void ElaThemePrivate::_applySystemTheme()
{
	Q_Q(ElaTheme);
	const ElaThemeType::ThemeMode mode = _detectSystemThemeMode();
	if (_themeMode != mode)
	{
		q->setThemeMode(mode);
	}
